...
void ABaseAIController::SetAwarnessState(EPlayerAwarenessState state)
{ 
    if(CurrentState == state)
    return;

    CurrentState = state; 
    Blackboard->SetValueAsInt(FName("Awarness"),static_cast<int>(state));
}
...
void ABaseAIController::OnTargetPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
    ...
    UE_LOG(LOGAI, Warning, TEXT("Perception updated for %s with %s at %s age:%f"), *Actor->GetName(), *Stimulus.Type.Name.ToString(), *Stimulus.StimulusLocation.ToString(), Stimulus.GetAge());
